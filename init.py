# -*- coding: utf-8 -*-
"""
Created on Sun May  2 15:27:27 2021
...
def init_stocks_classification():
    ...
    for k in classification:
        cursor.execute("INSERT INTO stocks_classification (classification_id, classification_name) VALUES (%s, %s) on conflict (classification_id) do nothing;", (k, classification[k]))
...
def init_stocks(classification_id): 
    ...
    for k in data["resualt"]:
        ...
        cursor.execute("INSERT INTO stocks (stock_id, classification_id, stock_name) VALUES (%s, %s, %s) on conflict (stock_id,classification_id) do nothing;", (k.split("\t")[0], classification_id, k.split("\t")[1]))
    conn.commit()
    cursor.close()
    conn.close()
